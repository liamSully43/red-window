...
export class OrangeComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more orange photos available",
  };
  page = 1;
  ...
  ngOnInit(): void {
    document.querySelector("title").innerHTML = "Red Window - Orange";
    let search = null;
    search = () => {
      fetch(`https://api.unsplash.com/search/photos/?&query=orange&per_page=30&page=${this.page}&order_by=popular&color=orange`, {
        "method": "GET",
        headers: {
