...
export class PurpleComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more purple photos available",
  };
  page = 1;
  ...
  ngOnInit(): void {
    document.querySelector("title").innerHTML = "Red Window - Purple";
    let search = null;
    search = () => {
      fetch(`https://api.unsplash.com/search/photos/?&query=purple&per_page=30&page=${this.page}&order_by=popular&color=purple`, {
        "method": "GET",
        headers: {
