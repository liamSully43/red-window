...
export class YellowComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more yellow photos available",
  };
  page = 1;
  ...
  ngOnInit(): void {
    document.querySelector("title").innerHTML = "Red Window - Yellow";
    let search = null;
    search = () => {
      fetch(`https://api.unsplash.com/search/photos/?&query=yellow&per_page=30&page=${this.page}&order_by=popular&color=yellow`, {
        "method": "GET",
        headers: {
