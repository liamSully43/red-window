...
export class BlackComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more black photos available",
  }
  page = 1;
  
  ngOnInit(): void {
    document.querySelector("title").innerHTML = "Red Window - Black";
    let search = null;
    search = () => {
      fetch(`https://api.unsplash.com/search/photos/?&query=black&per_page=30&page=${this.page}&order_by=popular&color=black`, {
        "method": "GET",
        headers: {
      ...
      .catch(() => {
        this.noResults = {
          state: true,
          message: "ItIt seems like something went wrong, please try again later.",
        }
      });
