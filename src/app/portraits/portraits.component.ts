...
export class PortraitsComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more portraits available",
  };
  page = 1;
  ...
  ngOnInit(): void {
    let search = null;
    search = () => {
    document.querySelector("title").innerHTML = "Red Window - Portraits";
      fetch(`https://api.unsplash.com/search/photos/?&query=portraits&per_page=30&page=${this.page}&order_by=popular`, {
        "method": "GET",
        headers: {
