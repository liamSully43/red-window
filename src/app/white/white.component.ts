...
export class WhiteComponent implements OnInit {
  ...
  noResults = {
    state: false,
    message: "That's it, there's no more white photos available",
  };
  page = 1;
  ...
  ngOnInit(): void {
    document.querySelector("title").innerHTML = "Red Window - White";
    let search = null;
    search = () => {
      fetch(`https://api.unsplash.com/search/photos/?&query=white&per_page=30&page=${this.page}&order_by=popular&color=white`, {
        "method": "GET",
        headers: {
